...
Cell** createCellGrid(int n)
{
    //int **grid = new int*;

    Cell **cell_grid = (Cell **) calloc( n , sizeof(Cell*)  );
    for (int i=0; i < n; i++){
        cell_grid[i] = (Cell *) calloc( n , sizeof(Cell) );
...
void solvePuzzleFila(int n, int **grid){
    ...
    Cell** last_position = createCellGrid(n);
    bool chegou = false;
    ...
    while(true){
        ...
        if(current_row+1 < n){
            if(grid[current_row+1][current_col] == 0){
                ...
                last_position[current_row+1][current_col].row = current_row;
                last_position[current_row+1][current_col].col = current_col;
            }
        }
        if(current_col+1 < n){
            if(grid[current_row][current_col+1] == 0){
                inserir(nova_fila,current_row,current_col+1);
                last_position[current_row][current_col+1].row = current_row;
                last_position[current_row][current_col+1].col = current_col;
            }
        }
        if(current_row-1 >= 0){
            if(grid[current_row-1][current_col] == 0){
    ...
    if(chegou)
    {
        ...
        int current_row = n-1;
        int current_col = n-1;
        empilhar(p, current_row, current_col);
        int new_pos_row = 0;
        int new_pos_col = 0;
        while(true)
        {
            new_pos_row = last_position[current_row][current_col].row;
            new_pos_col = last_position[current_row][current_col].col;

            current_row = new_pos_row;
            current_col = new_pos_col;
            empilhar(p, current_row, current_col);
            if(new_pos_row == 0 && new_pos_col == 0){
                break;
            }
        }
        while(p->topo != NULL){
            cout << "( " << p->topo->chave->row << ", " << p->topo->chave->col << ")" << endl;
            desempilha(p);
        }
    }
