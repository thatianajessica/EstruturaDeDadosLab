...
Arvore* createArvoreExample3()
{
    Arvore *arvore = criarArvoreVazia();

    NoTree* no1 = createNoTree2(5);
    NoTree* no2 = createNoTree2(50);
    NoTree* no3 = createNoTree2(30);
    NoTree* no4 = createNoTree2(39);
    NoTree* no5 = createNoTree2(32);

    connectNodes(no1, NULL, no2);
    connectNodes(no2, no3, NULL);
    connectNodes(no3, NULL, no4);
    connectNodes(no4, no5, NULL);
...
int compute_tamaho_arvore(NoTree *no_raiz){
    if(no_raiz->left == NULL && no_raiz->right == NULL)
    {
        return 1;
    }
    else if(no_raiz->left != NULL && no_raiz->right == NULL)
    {
        return compute_tamaho_arvore(no_raiz->left) + 1;
    }
    else if(no_raiz->left == NULL && no_raiz->right != NULL)
    {
        return compute_tamaho_arvore(no_raiz->right) + 1;
    }else
    {
        return compute_tamaho_arvore(no_raiz->left) + compute_tamaho_arvore(no_raiz->right) + 1;
    }
}

//10 15 20 30 40 50 60
void kesimo_menor(NoTree *no_raiz, int k, int &counter, bool &fim, int& valor_kesimo)
{
    ...
    if(fim == false){
        if(no_raiz->left==NULL && no_raiz->right==NULL)
        {
            ...
            if(k == counter)
            {
                valor_kesimo = no_raiz->chave_int;
                fim = true;
            }

            return;
        }
        if(no_raiz->left!=NULL) kesimo_menor(no_raiz->left, k, counter, fim, valor_kesimo);
        ...
        if(fim == false){
            ...
            if(no_raiz->right!=NULL) kesimo_menor(no_raiz->right, k, counter, fim, valor_kesimo);
        }
...
int main()
{
    ...
    Arvore* arvore3 = createArvoreExample3();
    int k=3;
    int counter = 0;
    bool fim = false;
    int valor_kesimo = 0;
    //kesimo_menor(arvore2->raiz, k, counter, fim, valor_kesimo);
    kesimo_menor(arvore3->raiz, k, counter, fim, valor_kesimo);

    cout << "valor_kesimo: " << valor_kesimo << endl;
